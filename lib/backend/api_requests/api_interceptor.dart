...
abstract class FFApiInterceptor {
  ...
  static Future<ApiCallResponse> makeApiCall(
    ...
    print('[FFApiInterceptor] makeApiCall: ${options.callName} → ${options.apiUrl}');
    // Update the options for each interceptor.
    for (final interceptor in interceptors) {
      try {
        options = await interceptor.onRequest(options: options);
      } catch (apiException) {
        print('[FFApiInterceptor] onRequest error: $apiException');
    ...
    // Make the API call.
    print('[FFApiInterceptor] executing HTTP request...');
    var response = await ApiManager.instance.call(options, client: client);
    print('[FFApiInterceptor] response received: statusCode=${response.statusCode}');
